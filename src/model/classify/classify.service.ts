...
export class ClassifyService {
  constructor(@InjectRepository(Classify) private readonly classifyrRepository: Repository<Classify>) { }
  ...
  async create(body: ClassifyDto): Promise<responseMsg<Classify, ClassifyDto>> {
    ...
    const isClassify = await this.classifyrRepository.findOne({ where: { classifyName } })
    if (isClassify) {
      return {
        message: "该类型名称已经被使用",
        statusCode: 500,
  ...
  async update(id: number, body: ClassifyDto): Promise<responseMsg<Classify, ClassifyDto>> {
    const { classifyName } = body;
    const isClassify = await this.classifyrRepository.findOne({ where: { classifyName } })
    if (isClassify) {
      return {
